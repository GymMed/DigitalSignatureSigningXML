        ...
        public static SuccessReport ValidateCertificateLocation(ApplicationFileFormats availableFormat = ApplicationFileFormats.PFX)
        {
            ...
            if (certificateLocation != "")
            {
                if (File.Exists(certificateLocation))
                {
                    ...
                    if (Path.GetExtension(certificateLocation).Equals(MainNavigationForm.fileFormatsExtensions[(int)availableFormat], StringComparison.OrdinalIgnoreCase))
                    {
                        report.Message = "Sertifikato lokacija tinkama!";
                        report.IsSuccessful = true;
                    }
                    else
                        report.Message = "Sertifikatas pateiktas netinkamu formatu! tinka tik: " + MainNavigationForm.fileFormats[(int) availableFormat];
                }
